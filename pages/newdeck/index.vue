...
#main
    .scene
        .leftSection
            .topSection
                .subTopSection
                    .deckStatus(:class='{deckFull: deckCreating.length == 30}') {{`牌組狀態：${deckCreating.length}/30`}}
                    .buttons
                        el-button.btnSave(@click='saveDeck' type='info' icon='el-icon-check' plain)
                            span Save
                        el-button.btnSave(@click='clearDeck' type='danger' icon='el-icon-delete' plain)
                            span Clear
                .subBottomSection
        ...
        .rightSection
            #createDeck
                v-drag#deckCards(
                    v-model='deckCreating'
                    :options='{group:"people"}'
                    @start='drag=true'
                    @end='drag=false'
                )
                    .msg(v-if='deckCreating.length == 0')
                        .text 拖曳卡牌至此
                            fa(icon='arrow-down' :style='{marginLeft: "5px"}')
                    .deckCard(v-for='card in deckCreating')
                        img(:src='`http://gametsg.techbang.com/hs/icon300/${card.img.split("/")[1]}.png`')
            #mydecks
</template>
...
export default {
    ...
    computed: {
        ...
        deckCreating: {
            get() {
                ...
                return this.$store.state.deckCreating.sort(compare)
            },
            set(value) {
                if(value.length <= 30) {
                    this.$store.commit('updateDeckCreating', value)
                } else {
    ...
    methods: {
        ...
        saveDeck() {
            let self = this
            if(this.deckCreating.length>24) {
                this.$prompt('命名你的牌組', '', {
                    confirmButtonText: '儲存',
                    cancelButtonText: '取消',
                    inputPlaceholder: '輸入牌組名稱'
                }).then(({value}) => {
                    let deckName;
                    if(!value) {
                        deckName = '我的牌組'
    ...
    mounted() {
        let self = this
        this.$root.$on('newDeckSaved', function() {
            self.clearDeck()
        })
    }
}
